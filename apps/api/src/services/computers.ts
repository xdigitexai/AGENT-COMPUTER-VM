...
export async function requestLifecycle(db:PrismaClient,queue:Queue<InfrastructureJobData>,input:{computerId:string;organizationId:string;action:LifecycleAction;idempotencyKey?:string}){
  return db.$transaction(async tx=>{
    ...
    if(input.action==="start"||input.action==="resume"){const [plan,running]=await Promise.all([tx.computePlan.findUnique({where:{id:computer.planId}}),tx.computer.count({where:{organizationId:input.organizationId,deletedAt:null,status:{in:["RUNNING","STARTING","REBOOTING"]}}})]);if(!plan||running>=plan.runningComputerLimit)throw Object.assign(new Error("Running computer quota exceeded"),{statusCode:409,code:"RUNNING_QUOTA_EXCEEDED"});}
