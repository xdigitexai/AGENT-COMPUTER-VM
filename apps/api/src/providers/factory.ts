...
export function providerFor(host: ComputeHost & { credential: ProviderCredential | null }, config: Config): VirtualizationProvider {
  ...
  if (host.provider.toLowerCase() === "docker") return new DockerProvider({endpoint:host.endpoint,network:credential.network??"xdigitex-computers",containerPrefix:credential.containerPrefix??"xdigitex-computer",volumePrefix:credential.volumePrefix??"agent-computer",pidsLimit:Number(credential.pidsLimit??512),stopTimeoutSeconds:Number(credential.stopTimeoutSeconds??20),desktopPort:Number(credential.desktopPort??6080),cdpPort:Number(credential.cdpPort??9222),geometry:credential.geometry??"1440x900",readonlyRootfs:String(credential.readonlyRootfs??"true")==="true"});
