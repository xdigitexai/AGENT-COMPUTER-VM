...
export interface CreateComputerInput { id: string; name: string; hostname: string; image: string; vcpu: number; ramMb: number; storageGb: number; sshPublicKey?: string; cloudInitUserData: string; }
export interface ResizeInput { vcpu?: number; ramMb?: number; storageGb?: number; }
export interface HostResources { cpuTotal: number; cpuUsed: number; ramTotalMb: number; ramUsedMb: number; storageTotalGb: number; storageUsedGb: number; }
